...
// Set fixed messaging API key for Mohamed's company (temporary endpoint)
routes.post('/api/set-messaging-key', async (req, res) => {
  try {
    ...
    if (result) {
      res.json({
        success: true,
        message: 'Fixed messaging API key set successfully',
        data: {
          company: result.name,
...
routes.get('/api/:session/qrcode-session', SessionController.getQrCode);
routes.get('/api/:session/qr', SessionController.getQrCode);
routes.get('/api/secret-key', SessionController.getSecretKey);
routes.post('/api/:session/start-session', SessionController.startSession);
routes.post(
...
// Webhook routes
routes.post('/webhook-test', WebhookController.webhookTest);
routes.post('/api/:session/webhook', verifyToken, WebhookController.setWebhook);
routes.get(
  '/api/:session/webhook-logs',
  verifyToken,
  WebhookController.getWebhookLogs
);

// Local FastAPI webhook configuration
routes.post('/api/webhook/configure', WebhookController.configureWebhook);
...
routes.get('/api/:session/status-session', SessionController.getSessionState);
routes.post(
