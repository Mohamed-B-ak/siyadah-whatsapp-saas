...
// Handle unhandled rejections from Puppeteer page closures
process.on('unhandledRejection', (reason, promise) => {
  logger.warn('[UNHANDLED-REJECTION] Promise rejection at:', promise);
...
logger.info(
  '[STARTUP] WPPConnect Server starting with autoClose: 0 configuration'
);
logger.info(
  '[STARTUP] Chrome executable path configured for Replit environment'
);
logger.info('[STARTUP] Enhanced logging enabled for session tracking');
...
export function initServer(serverOptions: Partial<ServerOptions>): {
  ...
  try {
    ...
    // Mount SaaS routes at /saas/api for dashboard compatibility
    app.use('/saas/api', saasRoutes);
    // Also mount at /api/v1 for unified API access
