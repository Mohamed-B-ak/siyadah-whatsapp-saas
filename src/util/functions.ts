...
// Import rate limiting functions from createSessionUtil
// We'll create a separate module for this to avoid circular dependencies
interface MessageRateLimit {
  lastMessageTime: number;
  messageCount: number;
  hourlyResetTime: number;
  eventCounts: { [eventType: string]: number };
}
...
const MESSAGE_RATE_LIMIT = {
  ...
  MAX_PER_EVENT_TYPE: 20, // Limit per event type
...
function shouldAllowMessageWebhook(sessionId: string, event: string, logger: any): boolean {
  ...
  if (!messageRateLimits[sessionId]) {
    messageRateLimits[sessionId] = {
      lastMessageTime: 0,
      messageCount: 0,
      hourlyResetTime: now + MESSAGE_RATE_LIMIT.RESET_INTERVAL,
      eventCounts: {},
    };
  }
...
export async function startAllSessions(config: any, logger: any) {
  try {
    ...
    }:${config.port}/api/${config.secretKey}/start-all`;
    await api.post(url);
  } catch (e) {
    logger.error(e);
  }
