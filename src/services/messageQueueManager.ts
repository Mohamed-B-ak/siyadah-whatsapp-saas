...
export class MessageQueueManager {
  ...
  async shouldQueueMessage(sessionId: string): Promise<boolean> {
    ...
    const canSendImmediately = await this.messageQueueService.canSendImmediately(sessionId);
  ...
  getSessionInfo(req: Request): {
    ...
    const companyId = req.body?.companyId || req.headers?.['x-company-id'] || 'unknown';
  ...
  async processMessageRequest(
    ...
    try {
      ...
      if (queueResult.queued) {
        ...
        req.logger?.info(`📬 Message queued for ${sessionInfo.sessionName}: ${phone} (ID: ${queueResult.messageId})`);
        ...
        try {
          ...
          req.logger?.info(`📤 Message sent immediately for ${sessionInfo.sessionName}: ${phone}`);
          ...
          req.logger?.error(`❌ Immediate send failed for ${sessionInfo.sessionName}: ${sendError.message}`);
  ...
  setMessageSender(sender: (sessionId: string, phone: string, message: string, options: any) => Promise<boolean>) {
    if (this.messageQueueService) {
      ...
      (this.messageQueueService).sendMessageViaWhatsApp = async (sessionId: string, messageTask: any) => {
        ...
        return await sender(sessionId, messageTask.phone, messageTask.message, messageTask.options);
      };
    }
  }
}
